...
function LSRPCore.Functions.GetPlayerData(cb)
    if not cb then return LSRPCore.PlayerData end
    cb(LSRPCore.PlayerData)
end
...
function LSRPCore.Functions.HasItem(items, amount)
    return exports['lsrp-inventory']:HasItem(items, amount)
end

-- Utility

function LSRPCore.Functions.DrawText(x, y, width, height, scale, r, g, b, a, text)
...
function LSRPCore.Functions.DrawText3D(x, y, z, text)
    -- Use local function instead
...
function LSRPCore.Functions.RequestAnimDict(animDict)
...
function LSRPCore.Functions.LookAtEntity(entity, timeout, speed)
...
function LSRPCore.Functions.PlayAnim(animDict, animName, upperbodyOnly, duration)
...
function LSRPCore.Functions.LoadModel(model)
...
function LSRPCore.Functions.LoadAnimSet(animSet)
...
RegisterNUICallback('getNotifyConfig', function(_, cb)
    cb(LSRPCore.Config.Notify)
end)

function LSRPCore.Functions.Notify(text, texttype, length, icon)
    local message = {
        action = 'notify',
...
function LSRPCore.Debug(resource, obj, depth)
    TriggerServerEvent('LSRPCore:DebugSomething', resource, obj, depth)
end

-- Callback Functions --

-- Client Callback
function LSRPCore.Functions.CreateClientCallback(name, cb)
    LSRPCore.ClientCallbacks[name] = cb
end

function LSRPCore.Functions.TriggerClientCallback(name, cb, ...)
    if not LSRPCore.ClientCallbacks[name] then return end
    LSRPCore.ClientCallbacks[name](cb, ...)
end

-- Server Callback
function LSRPCore.Functions.TriggerCallback(name, cb, ...)
    LSRPCore.ServerCallbacks[name] = cb
    TriggerServerEvent('LSRPCore:Server:TriggerCallback', name, ...)
end

function LSRPCore.Functions.Progressbar(name, label, duration, useWhileDead, canCancel, disableControls, animation, prop, propTwo, onFinish, onCancel)
    if GetResourceState('progressbar') ~= 'started' then error('progressbar needs to be started in order for LSRPCore.Functions.Progressbar to work') end
...
function LSRPCore.Functions.GetVehicles()
    return GetGamePool('CVehicle')
end

function LSRPCore.Functions.GetObjects()
...
function LSRPCore.Functions.GetPeds(ignoreList)
...
function LSRPCore.Functions.GetClosestPed(coords, ignoreList)
    local ped = PlayerPedId()
    if coords then
    ...
    ignoreList = ignoreList or {}
    local peds = LSRPCore.Functions.GetPeds(ignoreList)
...
function LSRPCore.Functions.IsWearingGloves()
    local ped = PlayerPedId()
    local armIndex = GetPedDrawableVariation(ped, 3)
    local model = GetEntityModel(ped)
    if model == `mp_m_freemode_01` then
        if LSRPCore.Shared.MaleNoGloves[armIndex] then
            return false
        end
    else
        if LSRPCore.Shared.FemaleNoGloves[armIndex] then
            return false
        end
    end
    return true
...
function LSRPCore.Functions.GetClosestPlayer(coords)
    local ped = PlayerPedId()
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
    ...
    local closestPlayers = LSRPCore.Functions.GetPlayersFromCoords(coords)
    local closestDistance = -1
...
function LSRPCore.Functions.GetPlayersFromCoords(coords, distance)
...
function LSRPCore.Functions.GetClosestVehicle(coords)
    local ped = PlayerPedId()
...
function LSRPCore.Functions.GetClosestObject(coords)
    local ped = PlayerPedId()
...
function LSRPCore.Functions.GetClosestBone(entity, list)
...
function LSRPCore.Functions.GetBoneDistance(entity, boneType, boneIndex)
    local bone
    if boneType == 1 then
...
function LSRPCore.Functions.AttachProp(ped, model, boneId, x, y, z, xR, yR, zR, vertex)
    local modelHash = type(model) == 'string' and joaat(model) or model
    local bone = GetPedBoneIndex(ped, boneId)
    LSRPCore.Functions.LoadModel(modelHash)
...
function LSRPCore.Functions.SpawnVehicle(model, cb, coords, isnetworked, teleportInto)
    local ped = PlayerPedId()
    ...
    LSRPCore.Functions.LoadModel(model)
...
function LSRPCore.Functions.DeleteVehicle(vehicle)
    SetEntityAsMissionEntity(vehicle, true, true)
    DeleteVehicle(vehicle)
end

function LSRPCore.Functions.GetPlate(vehicle)
    if vehicle == 0 then return end
    return LSRPCore.Shared.Trim(GetVehicleNumberPlateText(vehicle))
end

function LSRPCore.Functions.GetVehicleLabel(vehicle)
    if vehicle == nil or vehicle == 0 then return end
    return GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(vehicle)))
end

function LSRPCore.Functions.SpawnClear(coords, radius)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
...
function LSRPCore.Functions.GetVehicleProperties(vehicle)
    if DoesEntityExist(vehicle) then
        ...
        return {
            ...
            plate = LSRPCore.Functions.GetPlate(vehicle),
            plateIndex = GetVehicleNumberPlateTextIndex(vehicle),
            bodyHealth = LSRPCore.Shared.Round(GetVehicleBodyHealth(vehicle), 0.1),
            engineHealth = LSRPCore.Shared.Round(GetVehicleEngineHealth(vehicle), 0.1),
            tankHealth = LSRPCore.Shared.Round(GetVehiclePetrolTankHealth(vehicle), 0.1),
            fuelLevel = LSRPCore.Shared.Round(GetVehicleFuelLevel(vehicle), 0.1),
            dirtLevel = LSRPCore.Shared.Round(GetVehicleDirtLevel(vehicle), 0.1),
            oilLevel = LSRPCore.Shared.Round(GetVehicleOilLevel(vehicle), 0.1),
            color1 = colorPrimary,
...
function LSRPCore.Functions.SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
...
function LSRPCore.Functions.LoadParticleDictionary(dictionary)
...
function LSRPCore.Functions.StartParticleAtCoord(dict, ptName, looped, coords, rot, scale, alpha, color, duration)
    if coords then
        coords = type(coords) == 'table' and vec3(coords.x, coords.y, coords.z) or coords
    else
        coords = GetEntityCoords(PlayerPedId())
    end
    LSRPCore.Functions.LoadParticleDictionary(dict)
...
function LSRPCore.Functions.StartParticleOnEntity(dict, ptName, looped, entity, bone, offset, rot, scale, alpha, color, evolution, duration)
    LSRPCore.Functions.LoadParticleDictionary(dict)
    UseParticleFxAssetNextCall(dict)
...
function LSRPCore.Functions.GetStreetNametAtCoords(coords)
...
function LSRPCore.Functions.GetZoneAtCoords(coords)
    return GetLabelText(GetNameOfZone(coords))
end

function LSRPCore.Functions.GetCardinalDirection(entity)
...
function LSRPCore.Functions.GetCurrentTime()
    local obj = {}
...
function LSRPCore.Functions.GetGroundZCoord(coords)
...
function LSRPCore.Functions.GetGroundHash(entity)
    local coords = GetEntityCoords(entity)
